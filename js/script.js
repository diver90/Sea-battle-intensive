...
const fire = (event) => {
    ...
    if (target.classList.length !== 0 || target.tagName !== 'TD' || game.shipCount === play.dead) return;
    show.miss(target);
    ...
    for (let i = 0; i < game.ships.length; i++) {
        ...
        if (index >= 0) {
            ...
            if (life < 0) {
                ...
                if (game.shipCount === play.dead) {
                    header.textContent = 'Победа! Все корабли потоплены!';
